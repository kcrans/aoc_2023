...
while len(frontier) > 0:
    ...
    if last_axis == 'H':
        above = range(x - 4, max(x - 11, -1), -1 )
        below = range(x + 4, min(x + 11, m))
        next_cost = last_cost
        for next_x in range(x-1, max(x-4, -1), -1):
        ...
        next_cost = last_cost
        for next_x in range(x + 1, min(x + 4, m)):
            next_cost += loss_map[next_x][y]
        for next_x in below:
            next_cost += loss_map[next_x][y]
            if ((next_x, y, 'V')) not in seen:
        ...
        left = range(y - 4, max(y - 11, -1), -1)
        right = range(y + 4, min(y + 11, n))
        next_cost = last_cost
        for next_y in range(y - 1, max(y - 4, -1), -1):
        ...
        next_cost = last_cost
        for next_y in range(y + 1, min(y + 4, n)):
            next_cost += loss_map[x][next_y]
        for next_y in right:
            next_cost += loss_map[x][next_y]
            if ((x, next_y, 'H')) not in seen:
